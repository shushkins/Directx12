...
void LitColumnsApp::Draw(const GameTimer& gt)
{
	...
	auto curBufView = CurrentBackBufferView();
	auto dsv = DepthStencilView();
	// Specify the buffers we are going to render to.
	mCommandList->OMSetRenderTargets(1, &curBufView, true, &dsv);
	...
	auto bar1 = CD3DX12_RESOURCE_BARRIER::Transition(CurrentBackBuffer(),
        D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PRESENT);

    // Indicate a state transition on the resource usage.
    mCommandList->ResourceBarrier(1, &bar1);
...
void LitColumnsApp::UpdateMainPassCB(const GameTimer& gt)
{
	...
	auto projDetr = XMMatrixDeterminant(proj);
	XMMATRIX invProj = XMMatrixInverse(&projDetr, proj);
	auto viewDetr = XMMatrixDeterminant(viewProj);
	XMMATRIX invViewProj = XMMatrixInverse(&viewDetr, viewProj);
...
void LitColumnsApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
	...
	{
		auto ri = ritems[i];
		auto verBuffView = ri->Geo->VertexBufferView();
		auto indexBuffView = ri->Geo->IndexBufferView();

		cmdList->IASetVertexBuffers(0, 1, &verBuffView);
